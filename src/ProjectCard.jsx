...
export default function ProjectCard({ title, description, link, number = '01', tags = [] }) {
  return (
    <a href={link || "#"} target={link ? "_blank" : "_self"} rel="noopener noreferrer" className="project-card">
      <div className="card-bg">
        <svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 407 449" preserveAspectRatio="none">
          ...
          {/* Label divider line */}
          <line className="label-divider-line" x1="18" y1="175" x2="389" y2="175" strokeWidth="1.5"/>
        </svg>
      </div>

      {/* Decorative overlay: number badge + ruled lines only */}
      <div className="card-label-overlay">
        <div className="label-top-row">
          <span className="label-project-number">#{number}</span>
        </div>
        <div className="label-rule"/>
        <div className="label-lines">
          <div className="label-line-fake"/>
          <div className="label-line-fake label-line-fake--short"/>
          <div className="label-line-fake label-line-fake--medium"/>
          <div className="label-line-fake label-line-fake--shorter"/>
        </div>
      </div>

      {/* Card content: tags + title + description — always at bottom */}
